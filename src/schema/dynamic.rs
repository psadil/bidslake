...
impl Schema {
    ...
    fn generate_definitions(&mut self) {
        // Tables that come from `rules.tabular_data` are generated by one generic
        // path from the schema-driven [`Tabular`] model. This is what makes
        // `participants`, `sessions`, and `scans` correct: their columns (and the
        // true TSV header for each, e.g. `acq_time` not the schema key
        // `acq_time__scans`) are resolved from the schema rather than hand-listed.
        //
        // For now only the tables that already existed are emitted; the remaining
        // ~19 tabular tables are enabled in a later change once the `read_csv`
        // ingest path can populate them.
        let specs: Vec<TableSpec> = self.tabular.tables().to_vec();
        for spec in &specs {
            if matches!(spec.table.as_str(), "participants" | "sessions" | "scans") {
        ...
        // `events` stays on the legacy generator until the ingest switches to
        // `read_csv`: its bulk-load Appender path binds columns positionally, so
        // widening the table (bug #1's events half) has to land together with the
        // ingest change.
