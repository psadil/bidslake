...
def test_sql_tstring_is_injection_safe(lake):
    ...
    # so it matches nothing and the registry is left intact.
    evil = "bold'; DROP TABLE file_registry; --"
    matched = lake.sql(t"SELECT count(*) AS n FROM all_files WHERE suffix = {evil}")["n"][0]
    still_there = lake.sql("SELECT count(*) AS n FROM all_files")["n"][0]
