...
def test_every_catalog_table_has_a_model(lake) -> None:
    """A table that exists in a real catalog must have one, or a query cannot name it.

    The models are generated from the same introspection as `COLUMNS`, so the two cannot
    disagree without one of them being stale.
    """
...
def test_params_follow_the_placeholders_not_dict_order(three_equalities) -> None:
    """A compiled statement's params come back as a dict.

    Pairing them in any order but `positiontup` binds the wrong value to the wrong `?`,
    silently.
    """
...
def compiled_in_clause():
    """`in_()` builds an *expanding* param, and nothing on this path ever expands it.

    SQLAlchemy expands it when it executes the statement, which never happens here.
    Unexpanded, the SQL keeps a literal `__[POSTCOMPILE_x]` token and the param stays one
    nested list, so there is nothing for DuckDB to bind. No `lake` fixture: this needs no
    database, and the guard should still run where the bids-examples submodule is missing.
    """
...
def compiled_empty_in_clause():
    """The empty sequence takes a separate branch.

    SQLAlchemy renders `IN (NULL) AND (1 != 1)` rather than an empty `IN ()`, which is not
    valid SQL. Matching nothing is also what `get()` means by an empty sequence
    (`_compile_filters` emits `FALSE`), so both spellings of the query agree.
    """
...
def test_in_clause_matches_the_same_rows_as_get(lake) -> None:
    """The compiled form has to survive the round trip, not just look right.

    `get()`'s hand-rolled `IN` is the reference, and the statement must select the same
    files. Compared by `file_id` because `file_path` is dataset-relative and collides across
    the fixture's datasets, where a set comparison would quietly dedupe the difference away.
