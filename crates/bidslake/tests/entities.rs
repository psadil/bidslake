...
async fn null_session_for_sessionless_dataset() -> anyhow::Result<()> {
    ...
    let with_ses: i64 = db.conn.query_row(
        "SELECT COUNT(*) FROM all_files WHERE ses IS NOT NULL",
        [],
        |r| r.get(0),
    )?;
    ...
    let bold: i64 = db.conn.query_row(
        "SELECT COUNT(*) FROM all_files WHERE extension = '.nii.gz' AND suffix = 'bold' AND task = 'balloonanalogrisktask'",
        [],
        |r| r.get(0),
    )?;
    assert!(
...
async fn query_by_concept_across_mixed_pool() -> anyhow::Result<()> {
    ...
    let db = ingest(bids_example("ds210")).await?;
    {
        ...
        let mut parser = BidsParser::new(
            fs,
            Some("eyetracking_fmri".to_string()),
            schema,
            None,
            true,
            true,
    ...
    let rest: i64 = db.conn.query_row(
        "SELECT COUNT(*) FROM all_files WHERE extension = '.nii.gz' AND task = 'rest' AND datatype = 'func' AND suffix = 'bold'",
        [],
        |r| r.get(0),
    )?;
    assert!(
    ...
    let ses_states: i64 = db.conn.query_row(
        "SELECT COUNT(DISTINCT ses IS NULL) FROM all_files WHERE task = 'rest'",
        [],
        |r| r.get(0),
    )?;
    assert_eq!(
