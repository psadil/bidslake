...
async fn pseudofile_column_and_no_components() -> anyhow::Result<()> {
    ...
    let ds_pseudo: i64 = meg.conn.query_row(
        "SELECT COUNT(*) FROM all_files WHERE file_path LIKE '%_meg.ds' AND pseudofile",
        [],
        |r| r.get(0),
    )?;
    assert!(
    ...
    let components: i64 = meg.conn.query_row(
        "SELECT COUNT(*) FROM all_files WHERE file_path LIKE '%.ds/%'",
        [],
        |r| r.get(0),
    )?;
    assert_eq!(
    ...
    let nii_pseudo: i64 = mri.conn.query_row(
        "SELECT COUNT(*) FROM all_files WHERE suffix = 'bold' AND pseudofile",
        [],
        |r| r.get(0),
    )?;
    assert_eq!(
